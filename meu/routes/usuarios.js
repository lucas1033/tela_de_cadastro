...
router.post('/entrar', function (req, res, next) {
  ...
  banco.conectar().then(() => {
    ...
    return banco.sql.query(`select * from login where email='${login}' and senha='${senha}'`);
  }).then(consulta => {
...
router.post('/cadastrar', function (req, res, next) {
  ...
  banco.conectar().then(() => {
    ...
    if (nome == undefined || vingador == undefined || login == undefined || senha == undefined) {
      // coloque a frase de erro que quiser aqui. Ela vai aparecer no formulário de cadastro
      throw new Error(`Dados de cadastro não chegaram completos: ${nome} / ${vingador} / ${login} / ${senha}`);
    }
    return banco.sql.query(`select count(*) as contagem from login where email = '${login}'`);
  }).then(consulta => {
    ...
    if (cadastro_valido) {
      ...
      banco.sql.query(`insert into login (nome,vingador, email, senha) values ('${nome}','${vingador}','${login}','${senha}')`).then(function () {
